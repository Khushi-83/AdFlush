...
async function processNextMessage(payload, url) {
  // let tld=message['tld'];
  // let ct=message['type'];
  // console.log("payload",payload);
  let pred=await infer(payload);
  // console.log(pred, url,payload);//, payload);
  if(pred!==null){
    pred=pred[0];
    ...
    if(pred==1||pred=="True"){
      ...
      // console.log(blocked_url_numbers);
      ...
      // if('initiator' in details){
        ...
        try{
          ...
          if(localurl in time_dict){
            ...
            processNextMessage(dataA, localurl);
...
async function featureExtract(url, requestHeader){
  // console.log(url);
    ...
    let src_dom=requestHeader['originUrl'];
    if(src_dom!==undefined){
      src_dom=getDomain(src_dom);
    }
    else{
      src_dom=getDomain(url);
    }

    // console.log(src_dom,url);
      ...
      (async()=>{
          ...
          const [fqdnVector, reqVector] = await Promise.all([
            new Promise((innerResolve) => {
              browser.storage.local.get('fqdnvec', (storage) => {
                innerResolve(storage['fqdnvec']);
              });
            }),
            new Promise((innerResolve) => {
              browser.storage.local.get('reqvec', (storage) => {
...
browser.runtime.onInstalled.addListener(async()=>{
  browser.runtime.onMessage.addListener(async function(request, sender, sendResponse){
    ...
    if (request.action == "showBlocks") {
      console.log("showblocks",blocked_url_numbers,ext_time,inf_time);
      return {"block":blocked_url_numbers,"ext":ext_time,"inf":inf_time};
  ...
  .then(response=>{
      ...
      browser.storage.local.set({'fqdnvec':fqdnvector},function(){
          console.log("Saved fqdn vector on storage.", fqdnvector);
          return fqdnvector;
      });
  });
  ...
  .then(response=>{
      ...
      browser.storage.local.set({'reqvec':reqvector}, function(){
          console.log("Saved req vector on storage.", reqvector);
          return reqvector;
      });
  });

  try {
    // Load the model from the extension's file system
    ortsession = await ort.InferenceSession.create(browser.runtime.getURL("AdFlush.onnx"));
    // Send a message indicating the model has been loaded successfully
    console.log("ModelLoaded", ortsession.inputNames);
  } catch (e) {
    // If there is an error, send the error message back
    console.log("Model load error:", e.message );
  }

  // await setupOffscreenDocument('offscreen.html');

  browser.webNavigation.onBeforeNavigate.addListener(function(details){
    browser.tabs.query({currentWindow:true, active: true},function(){
  ...
  browser.storage.sync.get({"toggle":true},function(res){
    toggle=res.toggle;
...
// browser.declarativeNetRequest.onRuleMatchedDebug.addListener((response)=> {
...
async function infer(inputData){
  try {
    ...
    // Send error response if inference fails
